...
<%
    request.setCharacterEncoding("utf-8");
            String uidd=request.getParameter("userid");
            
            Integer int_uid=Integer.parseInt(uidd);
            Statement stmt=null,stmt2=null,stmt4=null,stmt5=null,stmt6=null;
            ...
            Integer loginuid=0;
            rs6=stmt6.executeQuery("select u_id from myweb.tb_user where u_name='"+nowuser+"'");
            if(rs6.next()){
                loginuid=rs6.getInt("u_id");
            }
            rs4=stmt4.executeQuery("select u_id,u_father_id from myweb.tb_user where u_id="+uid);
            Integer nowfa_id=0;
            if(rs4.next()){
                nowfa_id=rs4.getInt("u_father_id");
            }
            Integer now_id_rid=0;
            
            rs5=stmt5.executeQuery("select * from myweb.tb_role_user where u_id="+uid);
            if(rs5.next()){
                now_id_rid=rs5.getInt("r_id");
                System.out.println("r_id:"+now_id_rid);
            }
            if(loginuid.equals(int_uid)){
                
            rs=stmt.executeQuery("select id,username,time,position,IP,userid from myweb.logininfo where userid="+uid);
            System.out.println("uidshi2"+uid);
    ...
    <form action="fauser_log_op.jsp" id="f1">
        <div class="am-form-group" align="center">
              <div class="am-u-sm-9">
                <input type="text" id="name" name="userid" placeholder="输入ID查找">
              </div>
            </div>
        <div class="am-form-group">
            <div class="am-u-sm-9 am-u-sm-push-3" style="padding-left:60px;"><br/>
                <button type="button" onclick="sub();" name="Submit" value="提交" class="am-btn am-btn-primary" >点击查询</button>
              </div>
            </div>
        </form>
...
<td><span class="am-badge am-badge-success"><%=rs.getString("time")%></span></td>
...
else if(loginuid.equals(int_uid)==false&&now_id_rid.equals(3)&&nowfa_id.equals(loginuid)){
                rs=stmt.executeQuery("select id,username,time,position,IP,userid from myweb.logininfo where userid="+uid);
                System.out.println("uidshi3"+uid);
...
out.print("<script>alert('无查看权限...'); </script>");
out.print("<script>window.location='fauser_log.jsp' </script>");
}
    %>
