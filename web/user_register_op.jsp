...
%>
        ...
        try{
         ...
         ts.connect("smtp.163.com", "zichengwang123@163.com", "Aa123456");
            //创建邮件对象
            ...
            int rid=Integer.parseInt(roleidstr);//添加管理员
            ...
            rs7=stmt7.executeQuery("select * from myweb.tb_resource_role where r_id="+roleidstr);
            ...
            rs8=stmt8.executeQuery(sqlsu);
            if(rs8.next())
            {
            source_user_idadd=rs8.getInt("number");
            }
...
else if(address1.compareTo("")==0){
                out.print("<script>alert('地址不能为空'); </script>");
                out.print("<script>window.location='user_register.html' </script>");
            }  
else if(address2.compareTo("")==0){
                out.print("<script>alert('地址不能为空'); </script>");
                out.print("<script>window.location='user_register.html' </script>");
            }  
            %>
