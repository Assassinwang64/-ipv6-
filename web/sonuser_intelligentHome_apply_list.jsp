  ...
  <div class="admin-content">
    <div class="admin-content-body">
      <div class="am-cf am-padding">
        <div class="am-fl am-cf"><strong class="am-text-primary am-text-lg">您的家居列表</strong> / <small>模块</small></div>
      </div>
<div class="admin-content">
    <div class="admin-content-body">   
      <div class="am-g">
...
%>
            ...
            rs4=stmt4.executeQuery("select * from myweb.tb_user where u_name='"+nowuser+"'");
            if(rs4.next()){
                //nowuserid=rs4.getInt("u_id");
                faid=rs4.getInt("u_father_id");
            rs=stmt.executeQuery("select * from myweb.tb_user_ih where u_id="+faid);
            
             out.println("<div style='padding-top: 200px; padding-left: 500px;'>");
              out.println("<a href='#'>");
              out.println("</a>");
              out.println("</div>");
              out.println("<div style='color: #ffa186; padding-left: 500px;'>");
        %>
          ...
          <table class="am-table am-table-bd am-table-striped admin-content-table">
            ...
            <tbody>
                <tr>
                <% while(rs.next()){
                    Integer ihid2=rs.getInt("ih_id");
                    rs3=stmt3.executeQuery("select * from myweb.tb_intelligent_home where IHId="+ihid2);
                    rs3.next();
                    int id=rs.getInt("ih_id");
                    rs5=stmt5.executeQuery("select * from myweb.tb_ihresource_ih where ih_id="+id);
    %>
<td><%=rs.getInt("ih_id")%></td>
<td><%=rs3.getString("IHName")%></td>
<td><%=rs.getString("user_ih_operate_char")%></td>
<td>
    <form id="f1" action="sonuser_intelligentHome_apply_list_op.jsp" method="post">
                <div class="am-dropdown" data-am-dropdown>
                    
                  <select name="operate" id=""> 
                      <%while(rs5.next()){
                          int ihrid3=rs5.getInt("ihresource_id");
                          rs6=stmt6.executeQuery("select * from myweb.tb_intelligent_home_resource where IHRId="+ihrid3);
                          if(rs6.next()){
                              String str=rs6.getString("IHName");%>
                    ...
                    <button onclick="sub();" name="Submit" value="提交" class="am-btn am-btn-default am-btn-xs am-dropdown-toggle" data-am-dropdown-toggle>  操作</button>
                    <input readonly type="hidden" name="ihid" value="<%=ihid2%>">
                    <input readonly type="hidden" name="uid" value="<%=faid%>">
    </form>
              </td>
</tr>
    <%
